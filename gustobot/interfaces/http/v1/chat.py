...
from gustobot.application.agents.dietary_profile import (
    DietaryProfile,
    format_dietary_block,
    parse_dietary_profile,
)
from gustobot.application.agents.meal_plan import (
    MealPlan,
    build_ingredient_meal_query,
    format_user_display_message,
)
...
class ChatRequest(BaseModel):
    ...
    confirmed_ingredients: Optional[List[str]] = Field(
        None,
        description="User-confirmed ingredient list after photo recognition",
    )
    meal_plan: Optional[MealPlan] = Field(
        None,
        description="Meal planning scope: today_lunch, today_dinner, or week",
    )
    household_size: Optional[int] = Field(
        None,
        ge=1,
        le=12,
        description="Number of people eating; used to determine dishes per meal",
    )
    dietary_profile: Optional[DietaryProfile] = Field(
        None,
        description="Single-user diet preferences and health constraints (client-side profile)",
    )
...
async def process_agent_query(message: str, session_id: str,
                            ...
                            dietary_profile: Optional[DietaryProfile] = None) -> Dict[str, Any]:
    ...
    people = household_size or 2
    dietary_block = format_dietary_block(parse_dietary_profile(dietary_profile))
    if confirmed_ingredients:
        cleaned = [item.strip() for item in confirmed_ingredients if item and item.strip()]
        if cleaned:
            plan: MealPlan = meal_plan or "today_dinner"
            agent_message = build_ingredient_meal_query(cleaned, plan, people, dietary_block)
    elif dietary_block:
    ...
    input_state = {
        "messages": [{"type": "human", "content": agent_message}]
    }
...
async def stream_agent_response(message: str, session_id: str,
                               ...
                               meal_plan: Optional[MealPlan] = None,
                               household_size: Optional[int] = None,
                               dietary_profile: Optional[DietaryProfile] = None) -> AsyncGenerator[str, None]:
...
async def chat(
    ...
    user_display = request.message
    if request.confirmed_ingredients:
        cleaned = [item.strip() for item in request.confirmed_ingredients if item and item.strip()]
        if cleaned:
            user_display = format_user_display_message(
                cleaned,
                request.meal_plan or "today_dinner",
                request.household_size or 2,
            )

    # Save user message
    await save_message(db, session_id, user_display, is_user=True)
    ...
    result = await process_agent_query(
        ...
        request.file_path,
        effective_incremental,
        request.confirmed_ingredients,
        request.meal_plan,
        request.household_size,
        request.dietary_profile,
    )
