...
## `kb-query` - 知识库查询（历史文化 / 饮食文化史）
当问题涉及菜谱或饮食的**历史、文化、典故、流派、食材传入史**时使用此类。
这类问题需要从知识库（Milvus 长文本 + PostgreSQL 结构化）检索。
典型场景：
- 菜谱历史典故（如"宫保鸡丁的历史典故"）
- 菜品/菜系文化背景（如"川菜的特点和历史背景"）
- 食材历史文化（如"蔬菜是什么时候传入中国的"、"为什么叫胡萝卜"）
- 古代饮食常识（如"古代有哪些蔬菜"、"铁锅何时普及"）
- 菜系流派介绍（如"鲁菜的发展历史"）
...
2. 对于"怎么做"、"步骤"、"需要什么食材/用量"这类问题，选择 `graphrag-query`
3. 对于"历史"、"典故"、"文化背景"、"食材何时传入/命名由来"这类问题，选择 `kb-query`
