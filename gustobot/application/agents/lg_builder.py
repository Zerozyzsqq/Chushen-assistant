...
from gustobot.application.agents.vision_client import analyze_image, normalize_vision_model
...
def _chat_completions_url(base_url: str) -> str:
    """Normalize vision/LLM base URL to a single chat/completions endpoint."""
...
def _resolve_media_path(raw_path: Optional[str]) -> Optional[Path]:
    """Resolve uploaded media paths to an on-disk file."""
    return resolve_upload_path(raw_path)


def _router_for_confirmed_ingredients(config: RunnableConfig, question_text: str) -> Optional[Router]:
    """When user confirmed ingredients, force graph retrieval for meal planning."""
    cfg = _extract_configurable(config)
    raw = cfg.get("confirmed_ingredients")
    if not raw:
        return None
    if isinstance(raw, str):
        ingredients = [part.strip() for part in raw.split(",") if part.strip()]
    elif isinstance(raw, list):
        ingredients = [str(item).strip() for item in raw if str(item).strip()]
    else:
        return None
    if not ingredients:
        return None
    logger.info("Detected confirmed_ingredients in config, forcing graphrag-query route")
    return Router(
        type="graphrag-query",
        logic="config override: confirmed ingredients meal planning",
        question=question_text,
    )


def _router_for_media(config: RunnableConfig, question_text: str) -> Optional[Router]:
    """When image/file is attached, bypass LLM router and force the media branch."""
    cfg = _extract_configurable(config)
    if cfg.get("image_path"):
        logger.info("Detected image_path in config, forcing image-query route")
        return Router(
            type="image-query",
            logic="config override: image_path attached",
            question=question_text,
        )
    if cfg.get("file_path"):
        logger.info("Detected file_path in config, forcing file-query route")
        return Router(
            type="file-query",
            logic="config override: file_path attached",
            question=question_text,
        )
    return None
...
async def analyze_and_route_query(
    ...
    ingredients_router = _router_for_confirmed_ingredients(config, question_text)
    if ingredients_router:
        logger.info(f"Ingredients override routing: {ingredients_router}")
        return {"router": ingredients_router}

    media_router = _router_for_media(config, question_text)
    if media_router:
        logger.info(f"Media override routing: {media_router}")
        return {"router": media_router}
...
def route_query(
    ...
    Args:
        ...
        config (RunnableConfig): 运行配置，含 image_path / file_path 等附件信息。
    ...
    """
    cfg = _extract_configurable(config)
    if cfg.get("confirmed_ingredients"):
        logger.info("route_query: confirmed_ingredients present -> create_research_plan")
        return "create_research_plan"
    if cfg.get("image_path"):
        logger.info("route_query: image_path present -> create_image_query")
        return "create_image_query"
    if cfg.get("file_path"):
        logger.info("route_query: file_path present -> create_file_query")
...
async def create_image_query(
    ...
    raw_image_path = _extract_configurable(config).get("image_path")
    resolved_image = _resolve_media_path(raw_image_path)
    image_path = str(resolved_image) if resolved_image else raw_image_path
    ...
    if not resolved_image:
        logger.warning("User Upload Image Path is missing or not found: %s", raw_image_path)
        return {
            "messages": [
                AIMessage(
                    content=(
                        "抱歉，服务器找不到刚上传的图片文件。请重新上传后再试。"
                        f"（路径：{raw_image_path or '空'}）"
                    )
                )
            ]
        }
    ...
    if not (settings.VISION_API_KEY or settings.LLM_API_KEY) or not (
        settings.VISION_BASE_URL or settings.LLM_BASE_URL
    ):
        return {
            "messages": [
                AIMessage(
                    content="抱歉，视觉模型未配置。请在 .env 中设置 VISION_API_KEY、VISION_BASE_URL、VISION_MODEL。"
                )
            ]
        }

    vision_model = normalize_vision_model(settings.VISION_MODEL)
    logger.info("Using Vision Model: %s for image: %s", vision_model, image_path)
    ...
    try:
        image_description = await analyze_image(
            ...
            user_query,
            system_prompt=(
                "你是一个专业的菜谱图像分析助手。请详细分析图片中的内容，"
                "特别关注菜品名称、食材、烹饪方法、摆盘等细节。"
            ),
        )
        ...
        hint = (
            "请确认 .env 中 VISION_MODEL 为视觉模型（如 glm-4.6v-flash、glm-4v-plus），"
            "不要用 glm-4.6 等纯文本模型。"
        )
        return {"messages": [AIMessage(content=f"抱歉，视觉识别失败：{exc}\n{hint}")]}
    except Exception as e:
