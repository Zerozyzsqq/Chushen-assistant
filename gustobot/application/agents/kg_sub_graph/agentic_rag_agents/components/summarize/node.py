...
from gustobot.application.agents.kg_sub_graph.agentic_rag_agents.components.summarize.prompts import (
    create_recipe_howto_prompt_template,
    create_summarization_prompt_template,
    create_today_meal_plan_prompt_template,
    create_week_meal_plan_prompt_template,
)

generate_summary_prompt = create_summarization_prompt_template()
recipe_howto_prompt = create_recipe_howto_prompt_template()
week_meal_plan_prompt = create_week_meal_plan_prompt_template()
today_meal_plan_prompt = create_today_meal_plan_prompt_template()
...
def _is_week_meal_plan_question(question: str) -> bool:
    text = question or ""
    return "【任务类型】一周家常菜谱规划" in text or "【规划范围】一周菜谱" in text


def _is_today_meal_plan_question(question: str) -> bool:
    text = question or ""
    return "【任务类型】单餐菜谱推荐" in text and "【食材确认】" in text


def _is_recipe_howto_question(question: str) -> bool:
    text = question or ""
    if _is_week_meal_plan_question(text) or _is_today_meal_plan_question(text):
        return False
    return any(keyword in text for keyword in _HOWTO_KEYWORDS)


def _summary_has_cooking_steps(text: str) -> bool:
    """Heuristic: numbered steps or explicit 做法 section."""
    if not text:
        return False
    if re.search(r"(?m)^\s*\d+[.、．)\]]\s+\S", text):
        return True
    if re.search(r"做法[：:]", text) and len(text) > 80:
        return True
    return False
...
def create_summarization_node(
    ...
    synthesize_recipe_howto = recipe_howto_prompt | llm | StrOutputParser()
    synthesize_week_plan = week_meal_plan_prompt | llm | StrOutputParser()
    synthesize_today_plan = today_meal_plan_prompt | llm | StrOutputParser()
    ...
    async def summarize(state: OverallState) -> Dict[str, Any]:
        ...
        if not results_text:
            return {"summary": "暂未从知识图谱检索到相关菜谱，请补充食材或更换关键词。", "steps": ["summarize"]}

        try:
            if _is_week_meal_plan_question(question):
                summary = await synthesize_week_plan.ainvoke(
                    {"results": results_text, "question": question}
                )
            elif _is_today_meal_plan_question(question):
                summary = await synthesize_today_plan.ainvoke(
                    {"results": results_text, "question": question}
                )
            elif _is_recipe_howto_question(question):
                summary = await synthesize_recipe_howto.ainvoke(
                    {"results": results_text, "question": question}
                )
                if not _summary_has_cooking_steps(summary):
        ...
        summary = (summary or "").strip() or results_text
