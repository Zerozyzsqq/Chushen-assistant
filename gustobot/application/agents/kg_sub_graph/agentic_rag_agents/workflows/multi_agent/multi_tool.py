...
def create_kb_multi_tool_workflow(
    ...
    *,
    ...
    scope_text = scope_description or (
        "菜谱文化知识库处理与中华饮食文化史相关的问题，包括但不限于：\n"
        "1. 菜谱、菜品的历史渊源、命名来历、地域流派、典故故事；\n"
        "2. 历史名人与菜谱之间的关联；\n"
        "3. 食材（蔬菜、调料、作物等）何时传入中国、命名由来、古代饮食常识（如张骞引进、番/胡/洋命名等）；\n"
        "4. 古代烹饪方式、器皿（如铁锅普及）等与饮食文化相关的背景知识。\n"
        "明确不在范围内：具体某道菜的烹饪步骤与用量、现代采购/供应链、营养剂量或医疗诊断、"
        "政治娱乐八卦等非饮食文化主题。"
    )
        ...
        [
            (
                ...
                (
                    ...
                    "## 服务范围\n"
                    '包括"菜谱历史/渊源/命名"、"历史名人与菜谱关系"、"菜系流派介绍"，'
                    '以及"食材/蔬菜/作物的历史传入与饮食文化史"（知识库文本 data/kb 的主要内容）。\n\n'
                    "## 本地知识源说明\n"
                    ...
                    "**5. 超出范围类（拒绝回答）**\n"
                    "   - 仅当问题明确要求具体烹饪步骤、用量火候、现代采购渠道、营养治疗等\n"
                    "   - 注意：询问蔬菜/食材「历史来源、何时传入中国」属于范围内，应使用 tools: ['postgres','milvus']\n"
        ...
        [
            (
                ...
                (
                    ...
                    "1. 讨论菜谱历史、饮食文化史、食材传入与命名由来、地域流派、典故或与名人的关联；"
                    "不要给出具体烹饪步骤、用量或医疗养生建议。\n"
        ...
        if should_try_postgres:
            if not postgres_search_url:
                ...
                try:
                    ...
                    # PostgreSQL 不可用时仍走 Milvus 兜底（即使路由只选了 postgres）
                    should_try_milvus = True
